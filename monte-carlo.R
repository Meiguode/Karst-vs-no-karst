...
check_flux_summary <- function(trends) {
  ...
  df <- trends$summary
  cat("\n=== ", trends$dataset_name, "===\n")
  cat("Overall mean Rₛ:", round(mean(df$mean_flux, na.rm=TRUE), 4), "Pg C/year\n")
  cat("Overall SD Rₛ:", round(sd(df$mean_flux, na.rm=TRUE), 4), "\n")
  ...
  if(trends$dataset_name == "Karst" && mean_flux >= 25 && mean_flux <= 45) {
    cat("✓ Ecologically realistic range for karst\n")
  } else if(trends$dataset_name == "NonKarst" && mean_flux >= 55 && mean_flux <= 85) {
    cat("✓ Ecologically realistic range for non-karst\n")
  } else if(trends$dataset_name == "Combined" && mean_flux >= 80 && mean_flux <= 110) {
    cat("✓ Ecologically realistic range for combined\n")
  } else if(!is.na(mean_flux)) {
    cat("✗ Ecologically unrealistic range\n")
  }
...
for(i in 1:length(final_means)) {
  ...
  if(!is.na(mean_val)) {
    if(names(final_means)[i] == "karst") {
      status <- ifelse(mean_val >= 25 & mean_val <= 45, "✓ ECOLOGICALLY REALISTIC", "✗ UNREALISTIC")
    } else if(names(final_means)[i] == "nonkarst") {
      status <- ifelse(mean_val >= 55 & mean_val <= 85, "✓ ECOLOGICALLY REALISTIC", "✗ UNREALISTIC")
    } else {
      status <- ifelse(mean_val >= 80 & mean_val <= 110, "✓ ECOLOGICALLY REALISTIC", "✗ UNREALISTIC")
    }
...
if(all(final_means >= c(25, 55, 80) & final_means <= c(45, 85, 110))) {
  cat("\n🎉 ALL ESTIMATES ECOLOGICALLY REALISTIC! 🎉\n")
} else {
  cat("\n⚠ Some estimates need adjustment\n")
}
...
cat("✓ Final results saved to 'results/' directory\n")
cat("✓ Trend statistics saved to 'results/trend_statistics_all_periods.csv'\n")
cat("✓ Visualizations saved to 'plots/' directory\n")
cat("✓ Debug files saved to 'debug/' directory\n")
...
for(i in 1:nrow(trend_stats)) {
  ...
  if(row$Dataset == "Karst" && row$Period %in% c("period1", "period2", "period3", "period_full")) {
    ...
    cat("Annual global Rₛ", direction, "at a rate of", round(abs(row$Slope_Pg_yr), 2), 
        "Pg C yr⁻¹ in", period_label, "(", significance, ")\n")
  }
}
...
cat("\n✓ Comprehensive trend summary saved to: results/comprehensive_trend_summary.csv\n")
...
for(i in 1:nrow(karst_rows)) {
  row <- karst_rows[i, ]
  direction <- ifelse(row$Slope_Pg_yr > 0, "increased", "decreased")
  cat("Annual global Rₛ", direction, "at a rate of", abs(row$Slope_Pg_yr), 
      "Pg C yr⁻¹ in", row$Period, "(t", row$DF, "=", row$T_Value, ", P", row$P_Value, ")\n")
}
...
for(period_key in c("period1", "period2", "period3", "period_full")) {
  ...
  if(length(karst_slope) > 0 && length(nonkarst_slope) > 0) {
    ...
    cat("\n", period_label, ":\n")
    cat("  Karst slope:", round(karst_slope, 3), "Pg C yr⁻¹\n")
    cat("  Non-Karst slope:", round(nonkarst_slope, 3), "Pg C yr⁻¹\n")
    cat("  Difference:", round(slope_difference, 3), "Pg C yr⁻¹")
    cat(" (", round(relative_difference, 1), "% difference)\n", sep="")
    
    if(abs(relative_difference) > 10) {
      if(relative_difference > 0) {
        cat("  → Karst regions show ", round(relative_difference, 1), "% higher rate of change\n", sep="")
      } else {
        cat("  → Non-Karst regions show ", round(abs(relative_difference), 1), "% higher rate of change\n", sep="")
      }
    }
  }
}
...
for(region in c("Karst", "NonKarst", "Combined")) {
  ...
  if(length(region_slopes) > 0) {
    ...
    cat("\n", region, "Regions:\n")
    cat("  Average Rₛ trend:", round(avg_slope, 3), "Pg C yr⁻¹\n")
    cat("  Average annual flux:", round(avg_flux, 1), "Pg C year⁻¹\n")
    
    # Interpret the trend
    if(abs(avg_slope) < 0.1) {
      cat("  → Relatively stable over time\n")
    } else if(avg_slope > 0) {
      cat("  → Overall increasing trend\n")
    } else {
      cat("  → Overall decreasing trend\n")
    }
  }
}
